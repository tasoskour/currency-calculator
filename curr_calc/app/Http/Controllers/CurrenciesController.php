...
$validator = Validator::make($request->all(),[
      ...
      'required',
      'max:25',
      /*Create only for unique combination of target and base currency*/
     ...
     'rate' => 'required|numeric|between:0.000001,99999.999999',
   ]);
...
public function update(Request $request,ExchangeRate $exchangeId){
  ...
  $validator = Validator::make($request->all(),[
      ...
      'required',
      'max:25',
       /*Update only with unique names for the currencies*/
...
public function delete(ExchangeRate $exchangeId){
  $exchangeId->delete();
  $deleteMsg=array("Deleted successfully");
   return $deleteMsg;
}
}
